...
|[[input_cognito_user_pool_domain]] <<input_cognito_user_pool_domain,cognito_user_pool_domain>>
|Domain prefix of the Cognito user pool to use (custom domain currently not supported!). If not provided, the module will create its own pool and get its domain.
|`string`
|`null`
|no

|[[input_cognito_user_pool_id]] <<input_cognito_user_pool_id,cognito_user_pool_id>>
|ID of the Cognito user pool to use. If not provided, the module will create its own pool where to put the remaining created resources.
|`string`
|`null`
|no

|[[input_create_pool]] <<input_create_pool,create_pool>>
|Boolean to activate the creation of the pool. If set as true you'll cannot pass specify the variables `cognito_user_pool_id` and `cognito_user_pool_domain`.
|`bool`
|`false`
|no
